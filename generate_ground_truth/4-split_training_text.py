...
output_directory = "./amh-layer-ground-truth-4"
...
def parse_txt2img_log(line_no, font_name, output_base, result, is_beginning):
    ...
    if is_beginning:
        ...
        for f in {"split_out", "skipped_err", "stripped_err", "split_err"}:
            with open(f"./log4/{f}", "a") as log_file:
                log_file.write(f"\n------- {t} -------\n")

    # normal output
    if result.stderr:
        with open("./log4/split_out", "a") as splt_out:
    ...
    if f"{output_base}.tif" not in result.stderr:
        with open("./log4/skipped_err", "a") as err_file:
            err_file.write(f"File: {output_base}\nErr: {result.stderr}\n")

    # stripped words due to font error
    if "Stripped" in result.stderr:
        with open("./log4/stripped_err", "a") as err_file:
            err_file.write(f"File: {output_base}\nErr: {result.stderr}\n")

    # exceptions
    if result.returncode != 0:
        with open("./log4/split_err", "a") as err_file:
            err_file.writelines(
                [
