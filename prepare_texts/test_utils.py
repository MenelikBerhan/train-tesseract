...
from test_constants import allowed_non_eth_chars, allowed_non_eth_avoid, \
    enclosure_end_list, enclosure_start_list, eth_avoid, eth_puncs, \
    eth_unicode_range_all, eth_unicode_range_marks, junk_content_ptrn, \
    junk_enclosure_end_ptrns, junk_enclosure_start_ptrns, lines_to_skip, \
    same_enclosure_list, sentense_end_enclosures, \
    strip_chars_ptrn, strip_frm_start_and_end, to_keep_end, to_keep_start
...
def remove_word_with_unwanted_chars(match: re.Match):
    """
    Remove word if it contains non frequent without substitute.
    """
    word = match.group()
    ...
    for c in word:
        if not (
            c in allowed_non_eth_chars or
            (ord(c) not in eth_avoid and ord(c) in eth_unicode_range_all)
        ):
            return ''
