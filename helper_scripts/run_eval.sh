...
if [ "$(echo $1 | grep '\.checkpoint$')" ] ; then
	...
	if [ ! -f "$2/amh-layer.traineddata" ] ; then
	...
	lstmtraining --stop_training \
		--traineddata "$2"/amh-layer.traineddata  \
		--continue_from "$1" \
...
tesseract --tessdata-dir "$MODEL_PATH" "in_samples" "$MODEL"_result_samples -l "$MODEL"
tesseract --tessdata-dir "$MODEL_PATH" "in_kidane" "$MODEL"_result_kidane -l "$MODEL"
tesseract --tessdata-dir "$MODEL_PATH" "in_eval_6" "$MODEL"_result_eval_6 -l "$MODEL" --psm 7
...
lstmeval --model "$MODEL_PATH/$MODEL".traineddata --eval_listfile list.eval_5 --verbosity 2 >& eval_synth_"$MODEL"_5.log

# run lstmeval on lstmf files list of synthetic images from train trial 6
lstmeval --model "$MODEL_PATH/$MODEL".traineddata --eval_listfile list.eval_6 --verbosity 2 >& eval_synth_"$MODEL"_6.log
...
java -cp ocr-evaluation-tools/ocrevalUAtion-1.3.4-jar-with-dependencies.jar eu.digitisation.Main \
	-gt ground_samples.txt -e utf8  -ocr "$RES_SAMPLES" -e utf8 -o evlU_"$MODEL"_samples.html

java -cp ocr-evaluation-tools/ocrevalUAtion-1.3.4-jar-with-dependencies.jar eu.digitisation.Main \
	-gt ground_kidane.txt -e utf8 -ocr "$RES_KIDANE" -e utf8 -o evlU_"$MODEL"_kidane.html

#java -cp ocr-evaluation-tools/ocrevalUAtion-1.3.4-jar-with-dependencies.jar eu.digitisation.Main \
#	-gt ground_eval_6.txt -e utf8 -ocr "$RES_EVAL_6" -e utf8 -o evlU_"$MODEL"_eval_6.html

# run char & word accuracy, and synctext by comparing groundtruth with ocr results
accuracy ground_samples.txt "$RES_SAMPLES" acc_"$MODEL"_samples.txt
accuracy ground_kidane.txt "$RES_KIDANE" acc_"$MODEL"_kidane.txt
accuracy ground_eval_6.txt "$RES_EVAL_6" acc_"$MODEL"_eval_6.txt

wordacc ground_samples.txt "$RES_SAMPLES" wrd_acc_"$MODEL"_samples.txt
wordacc ground_kidane.txt "$RES_KIDANE" wrd_acc_"$MODEL"_kidane.txt
#wordacc ground_eval_6.txt "$RES_EVAL_6" wrd_acc_"$MODEL"_eval_6.txt

#synctext ground_samples.txt "$RES_SAMPLES" > sync_"$MODEL"_samples.txt
#synctext ground_kidane.txt "$RES_KIDANE" > sync_"$MODEL"_kidane.txt
#synctext ground_eval_6.txt "$RES_EVAL_6" > sync_"$MODEL"_eval_6.txt
